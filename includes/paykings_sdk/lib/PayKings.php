...
/**
 * Builds and sends an PayKings Payment Request.
 *
 * @package    PayKings Payment
 * @subpackage PayKings
 */
class PayKings extends PayKings_Request {
    private $live_url = 'https://paykings.transactiongateway.com/api/transact.php';
    private $test_url = 'https://paykings.transactiongateway.com/api/transact.php';
    ...
    /**
     * A list of all fields in the PayKings API.
     * Used to warn user if they try to set a field not offered in the API.
     */
	private $_all_paykings_fields = array(
    ...
    public function setField( $name, $value ) {
        if( $this->verify_fields ) {
            if( in_array( $name, $this->_all_paykings_fields ) ) {
                $this->_post_fields[$name] = $value;
            } else {
                throw new PayKings_Exception( "Error: no field $name exists in the PayKings API.
     ...
     *
     * @return PayKings_Response
     */
    protected function _handleResponse( $response ) {
        return new PayKings_Gateway_Response( $response );
    }
 ...
 * @subpackage PayKings
 */
class PayKings_Gateway_Response extends PayKings_Response {
    private $_response_array = array(); // An array with the split response.

    /**
     * Constructor. Parses the PayKings Payment response string.
     *
     * @param string $response      The response from the PayKings server.
     */
    public function __construct( $response ) {
        ...
        if( $response ) {
            ...
            /**
             * If PayKings Payment doesn't return a delimited response.
			*/
            if( count( $response_arr ) < 8 ) {
                $this->approved = false;
                $this->error = true;
                $this->error_message = sprintf( __( 'Unrecognized response from the gateway: %s', 'wc-paykings' ), $response );
                return;
            }
            ...
            if( $this->declined ) {
                $this->error_message = __( 'Your card has been declined.', 'wc-paykings' );
            }
            ...
            $this->approved = false;
            $this->error = true;
            $this->error_message = __( 'Error connecting to the gateway', 'wc-paykings' );
        }
    }
}